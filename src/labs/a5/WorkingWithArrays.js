...
function WorkingWithArrays() {
  ...
  const postTodo = async () => {
    // todo is the object to be posted to the server
    const response = await axios.post(API, todo);
    setTodos([...todos, response.data]);
  };

  // fetches the todos array from the server
  const fetchTodos = async () => {
    const response = await axios.get(API);
    setTodos(response.data);
  };

  //removes a todo from the todos array using routes from the server
  const removeTodo = async (todo) => {
    const response = await axios.get(`${API}/${todo.id}/delete`);
    setTodos(response.data);
  };

  // updates todos state variable
  ...
  // fetches a todo by id from the server
  const fetchTodoById = async (id) => {
    const response = await axios.get(`${API}/${id}`);
    setTodo(response.data);
  };

  // updates the title property of the todo object
  ...
  // uses setTodos to update the todos state variable to replace
  // the corresponding todo with the updated todo
  const updateTodo = async () => {
    try {
      const response = await axios.put(`${API}/${todo.id}`, todo);
      setTodos(todos.map((t) => (t.id === todo.id ? todo : t)));
      // set the todo state variable to an empty object
      setTodo({});
    } catch (error) {
      console.log(error);
      setErrorMessage(error.response.data.message);
